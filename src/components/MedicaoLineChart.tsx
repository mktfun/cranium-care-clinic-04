...
export function MedicaoLineChart({
  ...
  useEffect(() => {
    ...
    const processedData = [...medicoes].sort((a, b) => new Date(a.data).getTime() - new Date(b.data).getTime()).map(medicao => {
      const {
        months,
        days
      } = calculateAge(dataNascimento, medicao.data);
  ...
  const addIndiceReferenceData = (data: any[]) => {
    return data.map(point => ({
      ...
      // Atualizado para 85
      braquiLeve: 87,
      // Ajustado com base no novo limite superior
      braquiModerada: 90,
      dolicoLeve: 73,
      // Ajustado com base no novo limite inferior
      dolicoModerada: 70,
  ...
  const renderYAxis = () => {
    if (tipoGrafico === "indiceCraniano") {
      ...
      }} domain={[65, 95]} tickFormatter={value => `${value}%`} />;
      ...
      }} domain={[0, 12]} tickFormatter={value => `${value}%`} />;
    } else if (tipoGrafico === "diagonais") {
      return <YAxis label={{
        value: 'Diferença Diagonais (mm)',
        angle: -90,
        position: 'insideLeft',
        style: {
          textAnchor: 'middle'
        }
      }} domain={[0, 'dataMax + 2']} tickFormatter={value => `${value} mm`} />;
    } else if (tipoGrafico === "perimetro" || tipoGrafico === "perimetroCefalico") {
      return <YAxis label={{
        value: 'Perímetro Cefálico (cm)',
        angle: -90,
        position: 'insideLeft',
        style: {
          textAnchor: 'middle'
        }
      }} domain={['dataMin - 1', 'dataMax + 1']} tickFormatter={value => `${value} cm`} />;
    }
    return <YAxis />;
  };
  ...
  const renderLines = () => {
    ...
    if (tipoGrafico === "indiceCraniano") {
      return <>
          <Line type="monotone" dataKey="indice_craniano" name="Índice Craniano" stroke={lineColor} strokeWidth={3} dot={renderCustomDot(lineColor)} activeDot={{
          r: 7,
          fill: lineColor
        }} connectNulls isAnimationActive={true} />
          <Line type="monotone" dataKey="mediaPopulacional" name="Média Populacional" stroke="#666666" strokeDasharray="5 5" dot={false} connectNulls isAnimationActive={false} />
          <ReferenceLine y={76} stroke="#22C55E" strokeDasharray="3 3" label={{
          value: "Mín Normal",
          position: "insideBottomLeft"
        }} />
          <ReferenceLine y={80} stroke="#22C55E" strokeDasharray="3 3" label={{
          value: "Máx Normal",
      ...
      return <>
          <Line type="monotone" dataKey="cvai" name="CVAI" stroke={lineColor} strokeWidth={3} dot={renderCustomDot(lineColor)} activeDot={{
          r: 7,
          fill: lineColor
        }} connectNulls isAnimationActive={true} />
          <Line type="monotone" dataKey="mediaPopulacional" name="Média Populacional" stroke="#666666" strokeDasharray="5 5" dot={false} connectNulls isAnimationActive={false} />
          <ReferenceLine y={3.5} stroke="#22C55E" strokeDasharray="3 3" label={{
          value: "Limite Normal",
          position: "insideBottomLeft"
        }} />
        </>;
      ...
      return <>
          <Line type="monotone" dataKey="diferenca_diagonais" name="Diferença Diagonais" stroke={lineColor} strokeWidth={3} dot={renderCustomDot(lineColor)} activeDot={{
          r: 7,
          fill: lineColor
        }} connectNulls isAnimationActive={true} />
          <Line type="monotone" dataKey="mediaPopulacional" name="Média Populacional" stroke="#666666" strokeDasharray="5 5" dot={false} connectNulls isAnimationActive={false} />
          <ReferenceLine y={3} stroke="#22C55E" strokeDasharray="3 3" label={{
          value: "Limite Normal",
          position: "insideBottomLeft"
        }} />
        </>;
      ...
      return <>
          <Line type="monotone" dataKey="perimetro_cefalico" name="Perímetro Cefálico" stroke={lineColor} strokeWidth={3} dot={renderCustomDot(lineColor)} activeDot={{
          r: 7,
          fill: lineColor
        }} connectNulls isAnimationActive={true} />
          <Line type="monotone" dataKey="p3" name="P3" stroke="#9CA3AF" strokeDasharray="3 3" dot={false} connectNulls isAnimationActive={false} />
          <Line type="monotone" dataKey="p15" name="P15" stroke="#9CA3AF" dot={false} connectNulls isAnimationActive={false} />
          <Line type="monotone" dataKey="p50" name="P50" stroke="#4B5563" strokeWidth={2} dot={false} connectNulls isAnimationActive={false} />
          <Line type="monotone" dataKey="p85" name="P85" stroke="#9CA3AF" dot={false} connectNulls isAnimationActive={false} />
          <Line type="monotone" dataKey="p97" name="P97" stroke="#9CA3AF" strokeDasharray="3 3" dot={false} connectNulls isAnimationActive={false} />
        </>;
    }
  };
  ...
  const renderCustomTooltip = ({
    ...
    if (active && payload && payload.length > 0) {
      ...
      return <div className="bg-white/95 dark:bg-slate-800/95 p-3 border rounded shadow-lg">
  ...
  if (medicoes.length === 0) {
    return <div className={cn(`flex items-center justify-center w-full`, `h-[${altura}px]`, "border rounded-md bg-muted/30")}>
        <p className="text-muted-foreground">Nenhuma medição disponível</p>
      </div>;
  }
  return <div className="w-full">
      {(titulo || descricao) && <div className="mb-4">
          {titulo && <h3 className="text-lg font-medium">{titulo}</h3>}
          {descricao && <p className="text-sm text-muted-foreground\n\n<span class=\"hidden md:block text-sm text-muted-foreground\">\n  Esse texto aparece s\xF3 no desktop/tablet\n</span>">{descricao}</p>}
        </div>}
      <div style={{
      width: '100%',
      height: altura
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          if (e.activePayload && e.activePayload[0]) {
            ...
            <XAxis dataKey="idadeEmMeses" label={{
            value: 'Idade (meses)',
            position: 'insideBottomRight',
            offset: -15
          }} tickFormatter={formatXAxisTick} type="number" domain={[0, 'dataMax + 2']} allowDecimals={false} />
            {renderYAxis()}
