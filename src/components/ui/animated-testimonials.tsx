...
export function AnimatedTestimonials({
  ...
  return <section ref={sectionRef} id="testimonials" className={`py-24 overflow-hidden bg-muted/30 ${className || ""}`}>
        ...
        <motion.div initial="hidden" animate={controls} variants={containerVariants} className="grid grid-cols-1 gap-16 w-full md:grid-cols-2 lg:gap-24">
          ...
          <motion.div variants={itemVariants} className="flex flex-col justify-center">
            <div className="space-y-6">
              {badgeText && <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary/10 text-primary">
              ...
              <div className="flex items-center gap-3 pt-4">
                {testimonials.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`h-2.5 rounded-full transition-all duration-300 ${activeIndex === index ? "w-10 bg-primary" : "w-2.5 bg-muted-foreground/30"}`} aria-label={`View testimonial ${index + 1}`} />)}
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="relative h-full mr-10 min-h-[300px] md:min-h-[400px]">
            {testimonials.map((testimonial, index) => <motion.div key={testimonial.id} className="absolute inset-0" initial={{
            opacity: 0,
            x: 100
          }} animate={{
            ...
            zIndex: activeIndex === index ? 10 : 0
                ...
                <div className="bg-card border shadow-lg rounded-xl p-8 h-full flex flex-col">
                  <div className="mb-6 flex gap-2">
                    {Array(testimonial.rating).fill(0).map((_, i) => <Star key={i} className="h-5 w-5 fill-yellow-500 text-yellow-500" />)}
                  </div>
