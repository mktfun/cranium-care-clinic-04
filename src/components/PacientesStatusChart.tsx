...
export function PacientesStatusChart({
    ...
    async function carregarDados() {
      try {
        ...
        const {
          data: medicoes,
          error: medicoesError
        } = await supabase.from('medicoes').select('paciente_id, indice_craniano, cvai, data').order('data', {
          ascending: false
        });
        if (medicoesError) {
        ...
        const dadosFormatados: StatusData[] = Object.entries(statusDistribution).filter(([_, value]) => value > 0) // Só mostrar categorias com pacientes
        .map(([key, value]) => ({
  ...
  const gerarDadosSimulados = (): StatusData[] => [{
    ...
    name: 'Leve',
    value: 20,
    color: STATUS_COLORS.Leve
  }, {
    name: 'Moderada',
    value: 10,
    color: STATUS_COLORS.Moderada
  }, {
    name: 'Severa',
    value: 5,
    color: STATUS_COLORS.Severa
  }];
  const renderCustomizedLabel = ({
    ...
    return <text x={x} y={y} fill="white" textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" fontSize={isSmallScreen ? "10px" : "12px"} fontWeight="500">
