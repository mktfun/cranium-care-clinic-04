...
export function Header({ toggleSidebar, sidebarCollapsed, className, title }: HeaderProps) {
  ...
  return (
    <header
      className={cn(
        "border-b bg-background fixed top-0 right-0 left-0 z-10 transition-all duration-300 pt-safe",
        !isMobile && (sidebarCollapsed ? "lg:pl-[70px]" : "lg:pl-[250px]"),
        className
      )}
    >
      <div className="flex h-16 items-center px-4 gap-4">
        ...
        <div className="flex items-center gap-2">
          <Popover onOpenChange={(open) => { if (open) carregarNotificacoesReais(); }}>
